...
const ICON_BUTTON_CLASS =
  "text-content-brand-light hover:text-content-brand-light";

const DappSidebar = ({ dapp }: DappSidebarProps) => {
  const websiteUrl = dapp.websiteUrl ?? dapp.externalUrl;
  const websiteLabel = websiteUrl ? getDisplayHost(websiteUrl) : null;
  const hasAnySocial = Boolean(
    dapp.docsUrl || dapp.discordUrl || dapp.twitterUrl
  );
    ...
    <div className="max-w-102.5 w-full bg-surface-base-faint rounded-2xl sticky lg:top-26">
      ...
      <div className="divide-y divide-border-soft">
        <div className="px-4 md:px-6 py-4 md:py-6">
          <div className="relative h-14 md:h-20 w-14 md:w-20 inline-flex items-center justify-center rounded-full bg-surface-base-dark mb-2 md:mb-5">
            <Image
              src={dapp.iconSrc}
              alt={`${dapp.name} logo`}
              fill
              sizes="(min-width: 768px) 5rem, 3.5rem"
              className="rounded-full object-cover"
              unoptimized
            />
          </div>

          <h1 className="text-2xl md:text-4xl font-semibold leading-[1.2] tracking-[-0.36px] text-content-ultra mb-0.5">
            {dapp.name}
          </h1>
          {dapp.tagline ? (
            <p className="text-base md:text-lg leading-[1.7] tracking-[-0.18px] text-content-bold">
              {dapp.tagline}
            </p>
          ) : null}
          ...
          {websiteUrl ? (
            <div className="flex items-center gap-3 text-sm">
              ...
              <div className="space-y-0.5 min-w-0">
                <p className="text-xs leading-[1.2] tracking-[-0.12px] text-content-bold">
                  Website
                </p>
                <Link
                  href={websiteUrl}
                  target="_blank"
                  className="text-base font-medium leading-normal tracking-[-0.16px] text-content-brand-light break-all"
                >
                  {websiteLabel}
                </Link>
              </div>
          ...
          {dapp.listedOn ? (
            <div className="flex items-center gap-3 text-sm">
              ...
              <div className="space-y-0.5">
                <p className="text-xs leading-[1.2] tracking-[-0.12px] text-content-bold">
                  Listed on Lightchain Hub
                </p>
                <p className="text-base font-medium leading-normal tracking-[-0.16px] text-content-brand-strong">
                  {dapp.listedOn}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-base font-medium leading-normal tracking-[-0.16px] text-content-brand-strong flex items-center gap-2">
                  {dapp.projectId}
                  <CopyButton copyText={dapp.projectId} />
                </p>
              </div>
            </div>
          ) : null}
            ...
            <div className="grid grid-cols-3 gap-2 md:gap-4">
              ...
              {dapp.twitterUrl ? (
                <Button
                  ...
                  size="md"
                  className={ICON_BUTTON_CLASS}
                  aria-label={`${dapp.name} on X (Twitter)`}
                >
                  <span className="flex items-center justify-center w-6 md:w-7 h-6 md:h-7">
                    <TwitterIcon />
                  </span>
